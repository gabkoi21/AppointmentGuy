...
// node_modules/list/dist/es/index.js
...
function copyIndices(source, sourceStart, target, targetStart, length2) {
...
var List = function() {
  function List2(bits, offset, length2, prefix, root, suffix) {
    this.bits = bits;
    this.offset = offset;
    this.length = length2;
    this.prefix = prefix;
  ...
  List2.prototype.toJSON = function() {
    return toArray(this);
  };
  return List2;
}();
...
var ListIterator = /* @__PURE__ */ function() {
  function ListIterator2(l, direction) {
    this.l = l;
    this.result = { done: false, value: void 0 };
...
var ForwardListIterator = function(_super) {
  __extends(ForwardListIterator2, _super);
  function ForwardListIterator2(l) {
    return _super.call(this, l, 1) || this;
  }
  ForwardListIterator2.prototype.nextInTree = function() {
  ...
  ForwardListIterator2.prototype.next = function() {
    var newVal;
    var idx = ++this.idx;
  ...
  return ForwardListIterator2;
}(ListIterator);
var BackwardsListIterator = function(_super) {
  __extends(BackwardsListIterator2, _super);
  function BackwardsListIterator2(l) {
    return _super.call(this, l, -1) || this;
  }
  BackwardsListIterator2.prototype.prevInTree = function() {
  ...
  BackwardsListIterator2.prototype.next = function() {
    var newVal;
    var idx = --this.idx;
...
function push(value, l) {
  ...
  if (l.length === 0) {
    ...
    var newNode = new Node(void 0, l.suffix);
...
function from(sequence2) {
  var l = emptyPushable();
  if (sequence2.length > 0 && (sequence2[0] !== void 0 || 0 in sequence2)) {
    for (var i = 0; i < sequence2.length; ++i) {
      push(sequence2[i], l);
    }
  } else if (Symbol.iterator in sequence2) {
    var iterator = sequence2[Symbol.iterator]();
    var cur = void 0;
...
function range(start, end) {
  var list2 = emptyPushable();
  for (var i = start; i < end; ++i) {
    push(i, list2);
  }
  return list2;
}
function repeat(value, times2) {
  var l = emptyPushable();
  while (--times2 >= 0) {
    push(value, l);
  }
...
function nth(index, l) {
  ...
  var offset = l.offset;
  var depth = getDepth(l);
  return l.root.sizes === void 0 ? nodeNthDense(l.root, depth, offset === 0 ? index - prefixSize : handleOffset(depth, offset, index - prefixSize)) : nodeNth(l.root, depth, offset, index - prefixSize);
}
...
function affixPush(a, array, length2) {
  if (array.length === length2) {
    array.push(a);
    return array;
  } else {
    var newArray = [];
    copyIndices(array, 0, newArray, 0, length2);
    newArray.push(a);
    return newArray;
  }
}
...
function prependTopTree(l, depth, node) {
  ...
  if (l.root.array.length < branchingFactor) {
    newOffset2 = Math.pow(32, depth) - 32;
    l.root = new Node(void 0, arrayPrepend(createPath(depth - 1, node), l.root.array));
  } else {
    l.bits = incrementDepth(l.bits);
    var sizes = l.root.sizes === void 0 ? void 0 : [32, arrayLast(l.root.sizes) + 32];
    newOffset2 = depth === 0 ? 0 : Math.pow(32, depth + 1) - 32;
...
function prependNodeToTree(l, array) {
  if (l.root === void 0) {
    ...
    var node = new Node(void 0, array);
    var depth = getDepth(l);
    var newOffset_1 = 0;
...
function first(l) {
  var prefixSize = getPrefixSize(l);
  return prefixSize !== 0 ? l.prefix[prefixSize - 1] : l.length !== 0 ? l.suffix[0] : void 0;
}
var head = first;
function last(l) {
  var suffixSize = getSuffixSize(l);
  return suffixSize !== 0 ? l.suffix[suffixSize - 1] : l.length !== 0 ? l.prefix[0] : void 0;
}
...
function mapPrefix(f, prefix, length2) {
  var newPrefix = new Array(length2);
  for (var i = length2 - 1; 0 <= i; --i) {
    newPrefix[i] = f(prefix[i]);
  }
  return newPrefix;
}
function mapAffix(f, suffix, length2) {
  var newSuffix = new Array(length2);
  for (var i = 0; i < length2; ++i) {
...
function foldlSuffix(f, acc, array, length2) {
  for (var i = 0; i < length2; ++i) {
    acc = f(acc, array[i]);
  }
  return acc;
}
function foldlPrefix(f, acc, array, length2) {
  for (var i = length2 - 1; 0 <= i; --i) {
    acc = f(acc, array[i]);
  }
  return acc;
...
var reduce = foldl;
function traverse(of2, f, l) {
  return foldr(function(a, fl) {
    return fl["fantasy-land/ap"](f(a)["fantasy-land/map"](function(a2) {
      return function(l2) {
        return prepend(a2, l2);
      };
    }));
  }, of2["fantasy-land/of"](empty()), l);
}
...
function foldrSuffix(f, initial, array, length2) {
  var acc = initial;
  for (var i = length2 - 1; 0 <= i; --i) {
    acc = f(array[i], acc);
  }
...
function foldrPrefix(f, initial, array, length2) {
  var acc = initial;
  for (var i = 0; i < length2; ++i) {
    acc = f(array[i], acc);
  }
...
function foldlWhile(predicate, f, initial, l) {
  return foldlCb(foldlWhileCb, { predicate, f, result: initial }, l).result;
}
...
function find(predicate, l) {
  return foldlCb(findCb, { predicate, result: void 0 }, l).result;
}
function findLast(predicate, l) {
  return foldrCb(findCb, { predicate, result: void 0 }, l).result;
}
...
function findIndex(predicate, l) {
  var _a = foldlCb(findIndexCb, { predicate, found: false, index: -1 }, l), found = _a.found, index = _a.index;
...
function equalsWith(f, l1, l2) {
  if (l1 === l2) {
    ...
    var s = { iterator: l2[Symbol.iterator](), equals: true, f };
...
function rebalance(left, center, right, height, top) {
  ...
  var balanced = plan !== void 0 ? executeConcatPlan(merged, plan, height) : merged;
  if (balanced.length <= branchingFactor) {
    if (top === true) {
      return new Node(void 0, balanced);
    } else {
      return new Node(void 0, [
        setSizes(new Node(void 0, balanced), height)
      ]);
    }
  } else {
    return new Node(void 0, [
      setSizes(new Node(void 0, balanced.slice(0, branchingFactor)), height),
      setSizes(new Node(void 0, balanced.slice(branchingFactor)), height)
    ]);
  }
}
...
function appendNodeToTree(l, array) {
  ...
  if (nodesToCopy === 0) {
    ...
    var newRoot = new Node(void 0, [l.root, newPath]);
    l.root = newRoot;
    l.bits = incrementDepth(l.bits);
  } else {
...
function concatAffixes(left, right) {
  ...
  var arrIdx = 0;
  var i = 0;
  var length2 = getSuffixSize(left);
  concatBuffer[nr] = [];
  for (i = 0; i < length2; ++i) {
    concatBuffer[nr][arrIdx++] = left.suffix[i];
  }
  length2 = getPrefixSize(right);
  for (i = 0; i < length2; ++i) {
    if (arrIdx === 32) {
      arrIdx = 0;
      ++nr;
    ...
    concatBuffer[nr][arrIdx++] = right.prefix[length2 - 1 - i];
  }
  length2 = getSuffixSize(right);
  for (i = 0; i < length2; ++i) {
    if (arrIdx === 32) {
      arrIdx = 0;
      ++nr;
...
function sliceTreeList(from2, to, tree, depth, offset, l) {
  ...
  var _a = getPath(from2, offset, depth, sizes), pathLeft = _a.path, newFrom = _a.index;
  ...
  if (depth === 0) {
    ...
    if (pathLeft >= pathRight) {
      if (pathLeft > pathRight) {
        ...
        l.bits = decrementDepth(l.bits);
        var newRoot = childRight !== void 0 ? childRight : childLeft !== void 0 ? childLeft : tree.array[pathLeft];
...
function slice(from2, to, l) {
  var bits = l.bits, length2 = l.length;
  ...
  if (to <= from2 || to <= 0 || length2 <= from2) {
    return empty();
  }
  if (from2 <= 0 && length2 <= to) {
    return l;
  }
  ...
  var suffixStart = length2 - suffixSize;
  ...
  if (to < length2) {
    if (length2 - to < suffixSize) {
      bits = setSuffix(suffixSize - (length2 - to), bits);
      newList.suffix = l.suffix.slice(0, suffixSize - (length2 - to));
    } else {
...
function takeWhile(predicate, l) {
  var index = foldlCb(findNotIndexCb, { predicate, index: 0 }, l).index;
  return slice(0, index, l);
}
function takeLastWhile(predicate, l) {
  var index = foldrCb(findNotIndexCb, { predicate, index: 0 }, l).index;
...
function splitEvery(size, l) {
  var _a = foldl(function(_a2, elm) {
    var l22 = _a2.l2, buffer2 = _a2.buffer;
    push(elm, buffer2);
    if (buffer2.length === size) {
      return { l2: push(buffer2, l22), buffer: emptyPushable() };
    } else {
      return { l2: l22, buffer: buffer2 };
    }
  }, { l2: emptyPushable(), buffer: emptyPushable() }, l), l2 = _a.l2, buffer = _a.buffer;
  return buffer.length === 0 ? l2 : push(buffer, l2);
}
function remove(from2, amount, l) {
  return concat(slice(0, from2, l), slice(from2 + amount, l.length, l));
}
function drop(n, l) {
...
function sortWith(comparator, l) {
  ...
  forEach(function(elm) {
    return arr.push({ idx: i++, elm });
  }, l);
  arr.sort(function(_a, _b) {
    var a = _a.elm, i2 = _a.idx;
